import 'dart:math';

import 'package:flutter/material.dart';
import 'package:flutter_smart_home_ui/Models/admin_earning_defination.dart';
import 'package:flutter_smart_home_ui/provider/coupons_provider.dart';
import 'package:flutter_smart_home_ui/provider/meal_data.dart';
import 'package:flutter_smart_home_ui/students/screens/coupon.dart';
import 'package:provider/provider.dart';
import '../screens/cart.dart';

class FinalPayment extends StatefulWidget {
  static const routeName = '/final-Payment';
  generateRandomNumber() {

 var random = new Random();

    // Printing Random Number between 1 to 100 on Terminal Window.
    final couponNo = random.nextInt(100);
    print(random.nextInt(100));
    return couponNo;

}
  @override
  _FinalPaymentState createState() => _FinalPaymentState();
}

class _FinalPaymentState extends State<FinalPayment> {
  int selectedRadiaValue = 0;
  int selectedRadiaValue1 = 0;
  bool showBottomBar = false;

  void changeRadioGroupValue(int value) {
    setState(() {
      selectedRadiaValue = value;
      showBottomBar = true;
    });
  }

  @override
  Widget build(BuildContext context) {
    final mealData = Provider.of<MealDesData>(context);
    final couponProvider = Provider.of<Coupons>(context);
    final couponNumber = widget.generateRandomNumber();
    final routeArgs =
        ModalRoute.of(context)!.settings.arguments as Map<String, Object>;
    //final orderHistoryProvider = Provider.of<OrderHistoryData>(context);
    final totalAmount = routeArgs['totalPrice'] as int ;
    final totalBreakfast = routeArgs['totalBreakfast'] as int ;
    final totallunch = routeArgs['totallunch'] as int;
    final totalDinner = routeArgs['totalDinner'] as int;
    final totalOther = routeArgs['totalOther'] as int;
    final Map<String,dynamic> cartMealsData = routeArgs['cartMealsInfo'] as Map<String, dynamic>;
    // final orderedItemsData = routeArgs['OrderHistoryInfo'];
    // final cartMealsData = routeArgs['cartMealsInfo'];

    return Scaffold(
      appBar: AppBar(
        title: Text(
          'CheckOut',
          style: TextStyle(
            fontSize: 15,
            color: Colors.white,
            fontWeight: FontWeight.w700,
          ),
        ),
      ),
      bottomNavigationBar: showBottomBar
          ? Container(
              margin: EdgeInsets.only(left: 5, right: 5),
              width: double.infinity,
              height: MediaQuery.of(context).size.height * 0.07,
              child: RaisedButton(
                color: Color(0xff00001a),
                child: Text(
                  'Continue',
                  style: TextStyle(fontSize: 20, color: Colors.white),
                ),
                onPressed: () {
                  print('onPressed');
                  couponProvider.createCoupon(
                    cartMealsData, couponNumber

                  );
                  final currentData = Provider.of<MealDesData>(context, listen: false).admin;
                  Provider.of<MealDesData>(context, listen: false).adminDataPost(AdminEarnings(totalEarning: int.parse((currentData[0].totalEarning+totalAmount).toString()), breakfastEarn: currentData[0].breakfastEarn+totalBreakfast, dinnerEarn: currentData[0].dinnerEarn+totalDinner, lunchEarn: currentData[0].lunchEarn+totallunch, otherEarn: currentData[0].otherEarn+totalOther));
                  Navigator.of(context)
                      .popAndPushNamed(ConfirmationCoupon.routeName);
                },
              ),
            )
          : Container(
              margin: EdgeInsets.only(left: 5, right: 5),
              width: double.infinity,
              height: MediaQuery.of(context).size.height * 0.07,
              child: RaisedButton(
                color: Colors.grey,
                child: Text(
                  'Select an Payment Option',
                  style: TextStyle(fontSize: 20, color: Colors.white),
                ),
                onPressed: () {},
              ),
            ),
      body: SingleChildScrollView(
        child: Container(
          margin: EdgeInsets.all(10),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Container(
                margin: EdgeInsets.only(
                  bottom: 10,
                ),
                width: MediaQuery.of(context).size.width,
                height: MediaQuery.of(context).size.height * 0.1,
                child: Card(
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(10),
                  ),
                  color: Colors.amber[300],
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Container(
                        margin: EdgeInsets.only(
                          left: 5,
                        ),
                        child: Text(
                          'Total Amount: ',
                          style: TextStyle(
                            fontSize: 20,
                            color: Colors.black,
                          ),
                        ),
                      ),
                      Container(
                        margin: EdgeInsets.only(
                          right: 5,
                        ),
                        child: Text(
                          'Rs $totalAmount',
                          style: TextStyle(
                            fontSize: 25,
                            color: Colors.black,
                          ),
                        ),
                      ),
                    ],
                  ),
                ),
              ),
              Container(
                margin: EdgeInsets.only(left: 10),
                child: Text(
                  'Payment Options: ',
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    fontSize: 22,
                    fontWeight: FontWeight.bold,
                    color: Colors.black,
                  ),
                ),
              ),
              Container(
                margin: EdgeInsets.only(left: 10, top: 15),
                child: Text(
                  'UPI: ',
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
                    color: Colors.black,
                  ),
                ),
              ),
              GestureDetector(
                onTap: () {
                  setState(() {
                    showAlertDialog(context, selectedRadiaValue, 1);
                  });
                },
                child: Card(
                  elevation: 10,
                  color: Colors.white,
                  child: Container(
                    height: MediaQuery.of(context).size.height * 0.07,
                    margin: EdgeInsets.only(left: 5),
                    child: Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: Row(
                        children: <Widget>[
                          Radio(
                            value: 1,
                            groupValue: selectedRadiaValue,
                            onChanged: (value) {},
                          ),
                          Container(
                            decoration: BoxDecoration(
                              image: DecorationImage(
                                image: NetworkImage(
                                    'https://cdn-images-1.medium.com/max/1200/1*c6vHWp2F5UQTBM05UgQe8w.jpeg'),
                              ),
                            ),
                            height: MediaQuery.of(context).size.height * 0.08,
                            width: MediaQuery.of(context).size.width * 0.08,
                            margin: EdgeInsets.only(right: 8),
                          ),
                          Text(
                            'Paytm UPI',
                            style: TextStyle(
                              fontSize: 15,
                              color: Colors.black,
                            ),
                          ),
                        ],
                      ),
                    ),
                  ),
                ),
              ),
              GestureDetector(
                onTap: () {
                  setState(() {
                    showAlertDialog(context, selectedRadiaValue, 2);
                  });
                },
                child: Card(
                  elevation: 10,
                  color: Colors.white,
                  child: Container(
                    width: double.infinity,
                    height: MediaQuery.of(context).size.height * 0.07,
                    margin: EdgeInsets.only(left: 5),
                    child: Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: Row(
                        children: <Widget>[
                          Radio(
                            value: 2,
                            groupValue: selectedRadiaValue,
                            onChanged: (value) {},
                          ),
                          Container(
                            decoration: BoxDecoration(
                              image: DecorationImage(
                                  image: NetworkImage(
                                      'data:image/png;base64,')),
                            ),
                            height: MediaQuery.of(context).size.height * 0.08,
                            width: MediaQuery.of(context).size.width * 0.08,
                            margin: EdgeInsets.only(right: 8),
                          ),
                          Text(
                            'Google Pay',
                            style: TextStyle(
                              fontSize: 15,
                              color: Colors.black,
                            ),
                          ),
                        ],
                      ),
                    ),
                  ),
                ),
              ),
              GestureDetector(
                onTap: () {
                  setState(() {
                    showAlertDialog(context, selectedRadiaValue, 3);
                  });
                },
                child: Card(
                  elevation: 10,
                  color: Colors.white,
                  child: Container(
                    width: double.infinity,
                    height: MediaQuery.of(context).size.height * 0.07,
                    margin: EdgeInsets.only(left: 5),
                    child: Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: Row(
                        children: [
                          Radio(
                            value: 3,
                            groupValue: selectedRadiaValue,
                            onChanged: (value) {},
                          ),
                          Container(
                            decoration: BoxDecoration(
                              image: DecorationImage(
                                  image: NetworkImage(
                                      'https://pbs.twimg.com/profile_images/814869197025148928/sdmlM4R-_400x400.jpg')),
                            ),
                            height: MediaQuery.of(context).size.height * 0.08,
                            width: MediaQuery.of(context).size.width * 0.08,
                            margin: EdgeInsets.only(right: 8),
                          ),
                          Text(
                            'bhim UPI',
                            style: TextStyle(
                              fontSize: 15,
                              color: Colors.black,
                            ),
                          ),
                        ],
                      ),
                    ),
                  ),
                ),
              ),
              Container(
                margin: EdgeInsets.only(left: 10, top: 15),
                child: Text(
                  'Credit/Debit Cards: ',
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
                    color: Colors.black,
                  ),
                ),
              ),
              GestureDetector(
                onTap: () {
                  showDialog(
                    context: context,
                    builder: (BuildContext context) {
                      return AlertDialog(
                        content: Container(
                          height: MediaQuery.of(context).size.height * 0.4,
                          child: Column(
                            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                            children: [
                              Text(
                                'Add Debit/Credit/ATM Card',
                                style: TextStyle(
                                  color: Colors.black,
                                  fontSize: 20,
                                  fontWeight: FontWeight.w500,
                                ),
                              ),
                              Container(
                                width: MediaQuery.of(context).size.width * 0.8,
                                height:
                                    MediaQuery.of(context).size.height * 0.06,
                                child: TextField(
                                  style: TextStyle(color: Colors.black),
                                  keyboardType: TextInputType.phone,
                                  decoration: InputDecoration(
                                    hintText: 'Card Number',
                                    hintStyle: TextStyle(
                                      color: Colors.black54,
                                    ),
                                    enabledBorder: OutlineInputBorder(
                                      borderRadius: BorderRadius.all(
                                          Radius.circular(10.0)),
                                      borderSide:
                                          BorderSide(color: Colors.black),
                                    ),
                                    focusedBorder: OutlineInputBorder(
                                      borderRadius: BorderRadius.all(
                                          Radius.circular(10.0)),
                                      borderSide:
                                          BorderSide(color: Colors.black),
                                    ),
                                  ),
                                ),
                              ),
                              Row(
                                children: [
                                  Container(
                                    width:
                                        MediaQuery.of(context).size.width * 0.3,
                                    height: MediaQuery.of(context).size.height *
                                        0.06,
                                    child: TextField(
                                      style: TextStyle(color: Colors.black),
                                      keyboardType: TextInputType.phone,
                                      decoration: InputDecoration(
                                        hintText: 'Expiry MM',
                                        hintStyle: TextStyle(
                                          color: Colors.black54,
                                        ),
                                        enabledBorder: OutlineInputBorder(
                                          borderRadius: BorderRadius.all(
                                              Radius.circular(10.0)),
                                          borderSide:
                                              BorderSide(color: Colors.black),
                                        ),
                                        focusedBorder: OutlineInputBorder(
                                          borderRadius: BorderRadius.all(
                                              Radius.circular(10.0)),
                                          borderSide:
                                              BorderSide(color: Colors.black),
                                        ),
                                      ),
                                    ),
                                  ),
                                  Spacer(),
                                  Container(
                                    height: MediaQuery.of(context).size.height *
                                        0.06,
                                    width:
                                        MediaQuery.of(context).size.width * 0.3,
                                    child: TextField(
                                      style: TextStyle(color: Colors.black),
                                      keyboardType: TextInputType.phone,
                                      decoration: InputDecoration(
                                        hintText: 'CVV',
                                        hintStyle: TextStyle(
                                          color: Colors.black54,
                                        ),
                                        enabledBorder: OutlineInputBorder(
                                          borderRadius: BorderRadius.all(
                                              Radius.circular(10.0)),
                                          borderSide:
                                              BorderSide(color: Colors.black),
                                        ),
                                        focusedBorder: OutlineInputBorder(
                                          borderRadius: BorderRadius.all(
                                              Radius.circular(10.0)),
                                          borderSide:
                                              BorderSide(color: Colors.black),
                                        ),
                                      ),
                                    ),
                                  ),
                                ],
                              ),
                              Container(
                                height:
                                    MediaQuery.of(context).size.height * 0.06,
                                width: MediaQuery.of(context).size.width * 0.8,
                                child: TextField(
                                  style: TextStyle(color: Colors.black),
                                  keyboardType: TextInputType.phone,
                                  decoration: InputDecoration(
                                    hintText: 'Name on Card',
                                    hintStyle: TextStyle(
                                      color: Colors.black54,
                                    ),
                                    enabledBorder: OutlineInputBorder(
                                      borderRadius: BorderRadius.all(
                                          Radius.circular(10.0)),
                                      borderSide:
                                          BorderSide(color: Colors.black),
                                    ),
                                    focusedBorder: OutlineInputBorder(
                                      borderRadius: BorderRadius.all(
                                          Radius.circular(10.0)),
                                      borderSide:
                                          BorderSide(color: Colors.black),
                                    ),
                                  ),
                                ),
                              ),
                              Container(
                                width: double.infinity,
                                height:
                                    MediaQuery.of(context).size.height * 0.07,
                                child: RaisedButton(
                                  color: Color(0xff00001a).withOpacity(0.8),
                                  child: Text(
                                    'Continue',
                                    style: TextStyle(
                                      color: Colors.white,
                                      fontSize: 15,
                                    ),
                                  ),
                                  onPressed: () {
                                    
                                    setState(() {
                                      changeRadioGroupValue(5);
                                    });
                                    //mealData.removeAllCartItems(cartMealsData);
                                    Navigator.of(context).pop();
                                  },
                                ),
                              )
                            ],
                          ),
                        ),
                      );
                    },
                  );
                },
                child: Card(
                  elevation: 10,
                  color: Colors.white,
                  child: Container(
                    width: double.infinity,
                    height: MediaQuery.of(context).size.height * 0.07,
                    margin: EdgeInsets.only(left: 5),
                    child: Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: Row(
                        children: [
                          Radio(
                            value: 5,
                            groupValue: selectedRadiaValue,
                            onChanged: (value) {},
                          ),
                          Container(
                            decoration: BoxDecoration(
                              image: DecorationImage(
                                  image: NetworkImage(
                                      'https://icons-for-free.com/iconfiles/png/512/credit+card+debit+card+master+card+icon-1320184902602310693.png')),
                            ),
                            height: MediaQuery.of(context).size.height * 0.08,
                            width: MediaQuery.of(context).size.width * 0.08,
                            margin: EdgeInsets.only(right: 8),
                          ),
                          Text(
                            'Pay with Credit/Debit Card',
                            style: TextStyle(
                              fontSize: 15,
                              color: Colors.black,
                            ),
                          ),
                        ],
                      ),
                    ),
                  ),
                ),
              ),
              Container(
                margin: EdgeInsets.only(left: 10, top: 15),
                child: Text(
                  'More: ',
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
                    color: Colors.black,
                  ),
                ),
              ),
              GestureDetector(
                onTap: () {
                  setState(() {
                    selectedRadiaValue != 6
                        ? selectedRadiaValue = 6
                        : selectedRadiaValue = 0;
                  });
                },
                child: Card(
                  elevation: 10,
                  color: Colors.white,
                  child: Container(
                    width: double.infinity,
                    height: MediaQuery.of(context).size.height * 0.07,
                    margin: EdgeInsets.only(left: 5),
                    child: Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: Row(
                        children: [
                          Radio(
                            value: 6,
                            groupValue: selectedRadiaValue,
                            onChanged: (value) {},
                          ),
                          Container(
                            decoration: BoxDecoration(
                              image: DecorationImage(
                                  image: NetworkImage(
                                      'https://image.flaticon.com/icons/png/512/1554/1554401.png')),
                            ),
                            height: MediaQuery.of(context).size.height * 0.08,
                            width: MediaQuery.of(context).size.width * 0.08,
                            margin: EdgeInsets.only(right: 8),
                          ),
                          Text(
                            'Cash On Delivery',
                            style: TextStyle(
                              fontSize: 15,
                              color: Colors.black,
                            ),
                          ),
                        ],
                      ),
                    ),
                  ),
                ),
              ),
            ],
          ),
        ),
      ),
    );
  }

  showAlertDialog(BuildContext context, int selectedRadiaValue, int value) {
    // Create button
    Widget okButton = FlatButton(
      child: Text(
        "Link Account",
        style: TextStyle(
          color: Colors.black,
        ),
      ),
      onPressed: () {
        changeRadioGroupValue(value);
        Navigator.of(context).pop();
      },
    );

    // Create AlertDialog
    AlertDialog alert = AlertDialog(
      title: Text(
        "Enter Linked Phone Number:",
        style: TextStyle(
          color: Colors.black,
        ),
      ),
      content: TextField(
        style: TextStyle(color: Colors.black),
        keyboardType: TextInputType.phone,
        decoration: InputDecoration(
          hintText: 'Ex: 1234567891',
          hintStyle: TextStyle(
            color: Colors.grey,
          ),
          enabledBorder: OutlineInputBorder(
            borderRadius: BorderRadius.all(Radius.circular(10.0)),
            borderSide: BorderSide(color: Colors.grey),
          ),
          focusedBorder: OutlineInputBorder(
            borderRadius: BorderRadius.all(Radius.circular(10.0)),
            borderSide: BorderSide(color: Colors.grey),
          ),
        ),
      ),
      actions: [
        okButton,
      ],
    );

    // show the dialog
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return alert;
      },
    );
  }
}
